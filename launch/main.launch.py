...
def generate_launch_description() -> LaunchDescription:
    ...
    simulation = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(challenge_pkg_dir, 'launch', 'projet.launch.py')),
        launch_arguments={
    ...
    # ONE controller owns /cmd_vel for both challenges.
    # No line_follow_node is launched here, so there is no command conflict and
    # no unreliable handoff between two ROS processes.
    main_node = Node(
        package='projet',
    ...
    return LaunchDescription([set_model, x_arg, y_arg, yaw_arg, simulation, main_node])
